...
|First die|Second die=2|Second die=3|
|:---|:---|:---|
|1|2|3|
...
$$\Pr(X=x) =\begin{cases}
\frac{x-1}{36} & x \in \{2,\dots,7\} \\
\frac{13-x}{36} & x \in \{8,\dots,12\} \\
\end{cases}$$
...
for (i in 1:N) {
    y <- 0
    x <- sample(seq(6), 1) + sample(seq(6), 1)
    first_x <- x
    y <- y+1
    z <- ifelse(x==2 | x==3 | x==12, 0, ifelse(x==7 | x==11, 1, first_x))
    while (z>1) {
        x <- sample(seq(6), 1) + sample(seq(6), 1)
        y <- y+1
