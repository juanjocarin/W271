...
```{r P1-modelSelection_2, echo = FALSE, results='asis'}
```

Thus, the model is:

$$\begin{aligned}\log(homeValue) =& \beta_0 + \beta_1 \cdot pctLowIncome + \beta_2 \cdot pupilTeacherRatio + \beta_3 \cdot nBedRooms \\&+ \beta_4 \cdot pollutionIndex + \beta_5 \cdot \log(distanceToCity) + \beta_6 \cdot withWater + \varepsilon \end{aligned}$$
...
+ when the percentage of low-income households in the neighborhood increases by 1, the median value of a house in that neighborhood decreases `r frmt(-100*stepwise.reg$coefficients['pctLowIncome'], 1)`%.
+ when the number of pupils per teahcer increases by 1, the median home value decreases by `r frmt(-100*stepwise.reg$coefficients['pupilTeacherRatio'], 1)`%.
+ an additional bedroom increases the value of the house by approximately `r frmt(100*stepwise.reg$coefficients['nBedRooms'], 1)`%.
+ when the pollution index increases by 1, the median home value decreases by `r frmt(-100*stepwise.reg$coefficients['pollutionIndex'], 1)`% (since the standard deviation of the Pollution Index is `r frmt(sd(houseValue$pollutionIndex), 1)`, an increase of 1 standard deviation in that index would decrease the value of the house by approximately  `r frmt(-100*stepwise.reg$coefficients['pollutionIndex']*sd(houseValue$pollutionIndex), 1)`%).
+ when the distance to the nearest city increases by 1% (i.e., almost `r frmt(mean(houseValue$distanceToCity)*.01, 1)` mile, since the mean of that variable is `r frmt(mean(houseValue$distanceToCity), 1)` miles), the median home value decreases by `r frmt(-100*stepwise.reg$coefficients['log_distanceToCity'], 1)`%.
+ the proximity to a water body increases the home value by `r frmt(100*stepwise.reg$coefficients['withWater'], 1)`%.

I.e., not only the variables are statistically significant but also have **practical significance**.

```{r P1-modelSelection_3, echo = FALSE, fig.height=4.5, fig.width=6, fig.cap = "Residual diagnostics"}
```
...
```{r P1-IV_2, echo = FALSE, results = "asis"}
```
...
```{r P1-IV_4, echo = FALSE, results = "asis"}
```
...
```{r P1-IV_6, echo = FALSE, results = "asis"}
```

The other coefficients remaind almost unchanged (though not exactly; our aim was to eliminate the bias introduced by `pollutionIndex` because of its potential realtion with omitted variables), and now the impact of `pollutionIndex` has increased: an increase of one point decreases the median home value by `r frmt(-100*miv$coefficients['pollutionIndex'], 1)`% (and again, since the standard deviation of the Pollution Index is `r frmt(sd(houseValue$pollutionIndex), 1)`, an increase of 1 standard deviation in that index would decrease the value of the house by approximately  `r frmt(-100*miv$coefficients['pollutionIndex']*sd(houseValue$pollutionIndex), 1)`%). The (robust) standard error has increased due to the use of IVs (not perfectly correlated with `pollutionIndex`) but it's still low enough so the coefficient is highly statistically significant.
...
```{r P2-timeplot, echo = FALSE, fig.height = 3.9, fig.width = 6, fig.cap = "Time series plot of the financial series"}
```

```{r P2-ACF_PACF, echo = FALSE, fig.height = 3.9, fig.width = 6, fig.cap = "ACF and PACF of the financial time series"}
```
...
```{r P2-ACF_PACF_2, echo = FALSE, fig.cap = "ACF of the differenced financial time series"}
```
...
Since this is a **financial** series, let's analyze the \textcolor{red}{return} (or relative increment), defined as:
...
$$\mathbf{(1-B) \log(x_t) = \log(x_t) - \log(x_{t-1})} = \log\left(\frac{x_t}{x_{t-1}}\right) = \Delta \log(x_t) \approx \frac{x_t}{x_{t-1}} - 1 = \mathbf{r_t}$$
...
Let's now plot the log and difference of logs (i.e., the \textcolor{red}{log return}) of the financial series, as well as the respective ACFs and PACFs:

```{r P2-timeplot_ret, echo = FALSE, fig.height = 5.5, fig.width = 6, fig.cap = "Time series plot of the financial series"}
```

\pagebreak

```{r P2-ACF_PACF_ret, echo = FALSE, fig.height = 5, fig.width = 6, fig.cap = "ACF and PACF of the log return of the financial time series"}
```
...
```{r P2-forecast_2, echo = FALSE, fig.height = 4, fig.width = 6, fig.cap = "36-step ahead forecasts of the financial time series based on an ARIMA(0,1,0) model fitted to the log of the data"}
```
...
```{r P2-cond_var, echo = FALSE, fig.height=2.5, fig.cap = "ACF of the squared residuals of the ARIMA(0,1,0) model fitted to the log of the financial series"}
```
...
The residuals of such model resemble a white noise (with constant variance!), so the \textcolor{red}{ARIMA(0,1,0)/GARCH(1,1) of the log} is a good fit.

```{r P2-garch_2, echo = FALSE, fig.height = 4.5, fig.width = 6, echo = FALSE, fig.cap = "ACF of the residuals and squared residuals of an ARIMA(1,1,3)/GARCH(1,1) model fitted to the U.S. inflation-adjusted average gas prices"}
```
...
```{r P2-garch_4, echo = FALSE, fig.height = 4.5, fig.width = 6, fig.cap = "36-step ahead forecasts of the financial time series based on an ARIMA(0,1,0)/GARCH(1,1) model fitted to the log of the data"}
```
...
But we haven't checked for \textcolor{red}{conditional heteroskedasticity} (volatility) yet, so the prediction intervals above might not be accurate. After checking the auto-correlations of the squared residuals of our model (see the 1st Figure in the following page) we find that many of them are significant, which indicates volatility, so we start applying a GARCH(1,1) model.
...
```{r P4-GARCH_3, fig.height = 4.5, fig.width = 6, echo = FALSE, fig.cap = "ACF of the residuals and squared residuals of an ARIMA(1,1,3)/GARCH(1,1) model fitted to the U.S. inflation-adjusted average gas prices"}
```
