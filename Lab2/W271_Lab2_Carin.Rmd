...
$\mathbf{\color{red}{E(Y)}} = E\left[E(Y|X)\right] = \displaystyle\int_\mathbb{X} E(Y|X) \cdot f_X(x) \cdot dx = \int_{x=0}^1 \cfrac{x}{2} \cdot 1 \cdot dx = \left[ \cfrac{x^2}{4}\right]_{x=0}^1 = \mathbf{\color{red}{\cfrac{1}{4}}}$
...
y <- runif(simulations, min=0, max=x)
par(mfrow = c(1, 2))
hist(x, main = "pdf(x)")
hist(y, main = "pdf(y|x)")
plot(density(x), xlim=c(0,1))
plot(density(y), xlim=c(0,1), main = 'pdf(y|x)')

y <- runif(simulations, min=0, max=.5)
plot(density(y), xlim=c(0,1), main = 'pdf(y|x=0.5)')
y <- runif(simulations, min=0, max=.1)
plot(density(y), xlim=c(0,1), main = 'pdf(y|x=0.1)')

pdf_x <- function(x) ifelse(x<1 & x>0, 1, 0)
pdf_y_given_x <- function(x,y) ifelse(y<x & y>0 & x<1 & x>0, 1/x, 0)

pdf_y <- function(y) 
  sapply(y, function(y) integrate(function(x) 
    pdf_y_given_x(x,y)*pdf_x(x), 0, 1)$value)
plot(sort(y), pdf_y(sort(y)), type = 'l', main = 'pdf(y)')
plot(sort(y), log(1/sort(y)), type = 'l', main = 'pdf(y)')
...
x <- y <- seq(0, .2, by = 0.01)
grid <- mesh(x, y)
z <- with(grid, pdf_x(x)*pdf_y_given_x(x,y))
par(mfrow = c(1, 2))
contour(x, y, z, asp=1)
persp3D(z = z, x = x, y = y)
persp3D(z = z, x = x, y = y, pi = 0, theta = 0)
persp3D(z = z, x = x, y = y, pi = 0, theta = 90)
...
contour(x, y, z2, asp=1)
persp3D(z = z2, x = x, y = y)

integrate(pdf_x, 0, 1)

pdf_xy <- function(x,y) pdf_x(x)*pdf_y_given_x(x,y)
integrate(function(y) sapply(y, function(y) integrate(function(x) 
  pdf_xy(x,y), 0, 1)$value), 0, 1)
```
