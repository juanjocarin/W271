...
## @knitr ex1-ARIMA_models
#code from text page 144, still trying to fix an error
...
best.arima.hw08<-get.best.arima(data.ts, maxord= c(2,2,2,2,2,2))
best.fit.hw08<- best.arima.hw08[[2]]
acf(resid(best.fit.hw08))
best.arima.hw08[[3]]
ts.plot(cbind(window(hw08.ts, start=1980), exp(predict(best.fit.hw08, 12)$pred)), lty =1:2)
