...
We tested several bin widths, though here only three (5, 10, and 20) are plotted---they're enough to show that the smaller the bin size, the closer the histogram looks to the density plot (which is closer to the normal distribution in this case).
...
```{r Question3-3-3, echo = TRUE, message = FALSE, warning = FALSE, fig.cap = "Histogram of birth weight (in ounces), using `ggplot` and bin width = 10", fig.width = 4, fig.height = 3}
```

```{r Question3-3-4, echo = TRUE, message = FALSE, warning = FALSE, fig.cap = "Histogram of birth weight (in ounces), using `ggplot` and bin width = 20", fig.width = 4, fig.height = 3}
```
