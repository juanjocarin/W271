...
One of the ways to check this assumption is by plotting the residuals ($\hat{u}$) against the fitted values ($\hat{y}$), or even against the regressor when only one is used. That plot (below) shows that the residuals change with the fitted values, which suggests that the assumption may **not** be **realistic** (as well as a possible non-linear relationship between `mrate` and `prate`).

```{r Question4-1, echo = FALSE, fig.width = 6, fig.height = 4, fig.cap = "Scatterplot of fitted values of the regressand against the residuals"}
```
...
The implication is that the OLS coefficients are **biased**. But though the assumption of zero-conditional mean ($E(u|x)=0$) is not met, the assumption of zero mean ($E(u)=0$) and zero correlation ($Cov(x,u)=0$) seems realistic, though they may be **consistent** at least:
...
\textcolor{red}{2nd evidence?}

\textcolor{red}{We can derive formulas for the sampling variances whose components are easy to characterize + We can conclude that the OLS estimators have smallest variance among *all* linear, unbiased estimators}
...
> As expected, the density plot of the residuals has a shape similar to the explained variable.
...
The implication of non-normality of the errors is two-fold: the OLS estimators wiil not have the smallest variance among *all* (linear or not) unbiased estimators (assuming the Gauss-Markov assumptions are met, which is not the case here), and the *t* and *F* statistics do not have *t* and *F* distributions, respectively, which makes hypothesis testing impossible\textcolor{red}{... though...}.
...
As shown in `r I(pasteLabel("Table", tableCount, "table-Q3"))` and the R output in [Question 3](#Q3)), the effect of `mrate` on `prate` is highly statistically significante: the exact *p*-value is $p = `r formatC(summary(model)$coefficient[2, 4], format = "e", digits = 1)`$.
