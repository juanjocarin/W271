...
```{r Question5, echo = FALSE, fig.width = 6, fig.height = 4, fig.cap = "Scale-location plot of the regression model"}
```

\label{scale-location-Q5}

\textcolor{red}{2nd evidence: scale-location plot}
...
Just for fun (and since the CLM assumptions are not met in this example), we also computed the robust standard errors (the function we used can be found in the .R file accompanying this report), which is even smaller for the slope coefficient: **`r frmt(RSEs(model)[2, 2])`**.
...
As shown in `r I(pasteLabel("Table", tableCount, "table-Q3"))` and the R output in [Question 3](#Q3)), the effect of `mrate` on `prate` is highly statistically significante: the exact *p*-value is $p = `r formatC(summary(model)$coefficient[2, 4], format = "e", digits = 1)`$; even lower if we use robust standard errors, as shown in `r I(pasteLabel("Table", tableCount, "table-Q7"))`: $p = `r formatC(RSEs(model)[2, 4], format = "e", digits = 1)`$).
